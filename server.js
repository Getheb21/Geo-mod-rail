...
const PROTOCOLS = {
    P1: Buffer.from("VHJvamFu", 'base64').toString(),       // Trojan
    P2: Buffer.from("VkxFU1M=", 'base64').toString(),       // VLESS
    P3: Buffer.from("U2hhZG93c29ja3M=", 'base64').toString(), // Shadowsocks
    P4: Buffer.from("VkxFU1M=", 'base64').toString()        // VMess
};
...
<body class="min-h-screen py-4 md:py-8 px-3 md:px-6 relative transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto relative z-10">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div class="glass-deep p-4 rounded-xl flex items-center justify-between">
            <div>
              <p class="text-[10px] text-slate-400 font-bold mb-1">SYSTEM UPTIME</p>
              <p id="uptime-val" class="text-md font-extrabold">${systemUptime}s</p>
            </div>
            <i class="fa-solid fa-clock text-slate-500 text-lg"></i>
          </div>
          <div class="glass-deep p-4 rounded-xl flex items-center justify-between">
            <div>
              <p class="text-[10px] text-slate-400 font-bold mb-1">RAM ALLOCATION</p>
              <p class="text-md font-extrabold">${ramAllocation} MB</p>
            </div>
            <i class="fa-solid fa-microchip text-slate-500 text-lg"></i>
          </div>
          <div class="glass-deep p-4 rounded-xl flex items-center justify-between">
            <div>
              <p class="text-[10px] text-slate-400 font-bold mb-1">GATEWAY ENGINE</p>
              <p class="text-md font-extrabold text-emerald-400">NODE_JS</p>
            </div>
            <i class="fa-brands fa-node-js text-emerald-500/40 text-lg"></i>
          </div>
          <div class="glass-deep p-4 rounded-xl flex items-center justify-between">
            <div>
              <p class="text-[10px] text-slate-400 font-bold mb-1">UDP SERVICE</p>
              <p class="text-md font-extrabold text-cyan-400">ENABLED</p>
            </div>
            ...
            <div class="p-4 md:p-6 border-b" style="border-color: rgba(255,255,255,0.1);">
                <div class="relative group">
                    ...
                    <input type="text" id="searchInput" 
                        ...
                        class="w-full bg-white/10 backdrop-blur-sm border rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:ring-2 transition-all" style="border-color: rgba(255,255,255,0.1);">
                </div>
            </div>
    ...
    <script>
        ...
        function getCountryFullName(countryCode) {
            ...
            try {
                ...
                return countryNameFormatter.of(upperCode) || countryCode;
            } catch (error) {
                return countryCode;
            }
        }
        ...
        async function fetchProxies() {
            try {
                const response = await fetch('/api/raw-proxies');
                const text = await response.text();
                ...
                allProxies = lines.map(line => {
                    ...
                    return { 
                        ...
                        isp, countryCode: country,
                        status: null, delay: null, speed: null
                    };
        ...
        function generateVmess(proxy) {
            ...
            const vmessObj = { v: "2", ps: "[VMess-TLS] " + proxy.country + " - " + proxy.isp, add: window.location.hostname, port: window.location.port || 80, id: uuid, aid: "0", scy: "zero", net: "ws", type: "none", host: window.location.hostname, path: path, tls: "none", sni: "" };
            return VMS_PRE + btoa(JSON.stringify(vmessObj));
        }

        function generateVless(proxy) {
            const path = encodeURIComponent('/' + proxy.ip + '=' + proxy.port);
            return VLS_PRE + uuid + "@" + host + "?encryption=none&security=none&type=ws&host=" + window.location.hostname + "&path=" + path + "#" + encodeURIComponent("[VLESS-WS] " + proxy.country);
        }

        function generateTrojan(proxy) {
            const path = encodeURIComponent('/' + proxy.ip + '=' + proxy.port);
            return TRJ_PRE + uuid + "@" + host + "?security=none&type=ws&host=" + window.location.hostname + "&path=" + path + "#" + encodeURIComponent("[Trojan-WS] " + proxy.country);
        }

        function generateShadowsocks(proxy) {
            const encodedAuth = btoa('none:' + uuid);
            const path = encodeURIComponent('/' + proxy.ip + '=' + proxy.port);
            return 'ss://' + encodedAuth + '@' + host + '?path=' + path + '&security=none&host=' + window.location.hostname + '&type=ws#' + encodeURIComponent("[SS-WS] " + proxy.country);
        }
        ...
        function getStatusHtml(proxy) {
            ...
            if (proxy.status === 'ACTIVE') {
                return '<div class="status-badge status-active tooltip" data-tooltip="Delay: ' + proxy.delay + '"><i class="fas fa-check-circle text-green-500"></i><span class="text-green-500 font-semibold">ACTIVE</span></div>';
            } else if (proxy.status === 'ERROR') {
                return '<div class="status-badge status-inactive"><i class="fas fa-exclamation-triangle"></i><span>ERROR</span></div>';
            } else {
        ...
        function renderTable() {
            ...
            paged.forEach((proxy, idx) => {
                ...
                tbody.innerHTML += '<tr class="border-b border-white/5 hover:bg-white/5 transition-all">' +
                    ...
                    '<button onclick="copyToClipboard(\\'' + shadowsocks + '\\', this)" class="bg-cyan-600 hover:bg-cyan-700 p-2 rounded-md text-[10px] font-bold text-white flex flex-col items-center gap-1"><i class="fas fa-lock"></i> SS</button>' +
        ...
        function updatePagination() {
            ...
            const btnClass = "px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-xs hover:bg-white/10 disabled:opacity-30 text-slate-300";
        ...
        function getFlagEmoji(countryCode) {
            ...
            const codePoints = countryCode.toUpperCase().trim().split('').map(char => 127397 + char.charCodeAt());
  ...
  async handleWebSocketConnection(ws, request) {
    try {
        ...
        ws.close(1011, 'Internal connection handler breakdown');
    }
  }
  ...
  async websocketTunnelHandler(ws, selectedProxy) {
    ...
    ws.on('message', async (message) => {
        try {
            ...
            // Inisialisasi Socket Outbound TCP (Node Native Sockets)
            ...
            console.error('Failed processing stream buffer:', err);
            ws.close(1011, 'Data processing exception');
        }
    });
    ...
    ws.on('error', () => {
        if (remoteSocketWrapper.value) remoteSocketWrapper.value.destroy();
    });
  }
